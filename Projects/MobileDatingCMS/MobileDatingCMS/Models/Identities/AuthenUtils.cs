...
namespace MobileDatingCMS.Models
{
    public static class AuthenUtils
    {
        ...
        public static AspNetUser GetUserAspNet(TempDataDictionary pTempData, IIdentity pIdentity, CmsEntities dc = null)
        {
            return GetUser(pTempData, pIdentity).ToAspNetUser(dc);
        }

        public static AspNetUser ToAspNetUser(this ApplicationUser pUser, CmsEntities dc = null)
        {
            if (dc == null)
            {
                dc = new CmsEntities();
